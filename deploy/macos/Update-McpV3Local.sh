...
[[ -x /usr/bin/tar ]] || mcp_v3_die "MCP V3 package dependency is missing: /usr/bin/tar"

current_release="$(mcp_v3_manifest_release_id "$node" "$manifest")"
platform_id="$(mcp_v3_host_platform darwin)"
asset_arch="${platform_id#darwin-}"
...
asset="$tag-darwin-$asset_arch.tar.gz"
tmp="$(mktemp -d)"
trap 'rm -rf -- "$tmp"' EXIT
mcp_v3_download_verified_asset "$node" "$repository" "$tag" "$asset" "$tmp/$asset"
mkdir -p "$tmp/unpacked"
/usr/bin/tar -xzf "$tmp/$asset" -C "$tmp/unpacked"
...
"$package_root/deploy/macos/Install-McpV3Local.sh" "${args[@]}"
