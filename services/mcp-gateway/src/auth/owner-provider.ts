...
function approvalFormHtml(params: {
  error?: string;
  resourceName: string;
  clientName: string;
  scopes: string[];
  ...
  const scopeText = params.scopes.length > 0 ? params.scopes.join(" ") : "mcp:tools";
  const resourceText = params.resource?.href ?? params.resourceName;
...
<head>
  ...
  <title>Autorizar ${htmlEscape(params.resourceName)}</title>
  <style>
    body { font-family: system-ui, sans-serif; margin: 0; background: #0f172a; color: #e2e8f0; }
    main { max-width: 440px; margin: 12vh auto; padding: 32px; background: #111827; border: 1px solid #334155; border-radius: 18px; }
    ...
    input[type=password] { width: 100%; padding: 12px; border-radius: 10px; border: 1px solid #475569; background: #020617; color: #e2e8f0; box-sizing: border-box; }
    button { margin-top: 18px; width: 100%; border: 0; border-radius: 10px; padding: 12px; font-weight: 700; background: #38bdf8; cursor: pointer; }
    .error { color: #fecaca; background: #7f1d1d; border-radius: 10px; padding: 10px 12px; }
  </style>
</head>
<body>
  <main>
    <h1>Conectar ${htmlEscape(params.resourceName)}</h1>
    <p>Aprove somente se voce esta conectando seu proprio ChatGPT ou cliente MCP a esta maquina.</p>
    ${error}
    <dl>
      <dt>Cliente</dt><dd>${htmlEscape(params.clientName)}</dd>
      <dt>Escopo</dt><dd>${htmlEscape(scopeText)}</dd>
      <dt>Recurso</dt><dd>${htmlEscape(resourceText)}</dd>
    </dl>
    <form method="post">
      ${hiddenFields}
      <label for="owner_token">Senha Owner</label>
...
export class OwnerOAuthProvider implements OAuthServerProvider {
  ...
  async authorize(
    ...
    const providedToken = String((res.req.body as { owner_token?: string })?.owner_token ?? "");
    if (!safeEquals(providedToken, this.config.ownerToken)) {
      res.status(401).setHeader("Content-Type", "text/html; charset=utf-8");
      res.send(
        approvalFormHtml({
          error: "A senha Owner nao foi aceita.",
          resourceName: this.config.resourceName,
          clientName: client.client_name ?? client.client_id,
          scopes: params.scopes ?? this.config.scopes,
          resource: params.resource,
...
export function deriveAllowedRedirectHosts(publicBaseUrl: URL): string[] {
  const hosts = new Set<string>();
  hosts.add(publicBaseUrl.hostname);
  return [...hosts];
}
