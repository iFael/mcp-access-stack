...
. (Join-Path $PSScriptRoot 'CredentialBroker.Common.ps1')

$commonSource = Get-Content -Raw -LiteralPath (Join-Path $PSScriptRoot 'CredentialBroker.Common.ps1')
foreach ($requiredCompilerTempContract in @(
    ...
    if (-not $commonSource.Contains($requiredCompilerTempContract)) {
        throw "Credential broker helper compilation must use the short system temporary path contract: $requiredCompilerTempContract"
    }
}
